...
title: tl;dr
- 가장 중요한 건 `바이트 코드`이다.
- 자바코드를 컴파일해서 바이트 코드가 생기면 JVM 내부 구조를 통해서 실행하게 된다
```
...
## 2. JVM 구조
![](../img/Pasted%20image%2020221020065430.png)
...
- 컴파일된 .class 에서 바이트 코드를 읽고 메모리에 적절히 배치하여 저장
	![|450](../img/Pasted%20image%2020221019063929.png)
...
## 3. [클래스 로더 시스템](클래스%20로더%20시스템.md)
